...
- 对单词、短语、短语动词、习语和搭配，AI 可以给出一小组真正的英文同义词/近义表达。它们只保存为当前词条的 `synonyms` 元数据，在公开卡片、详情、复制、搜索和 JSON 导出中显示，绝不会自动建立额外词条，也不参与重复词判断；以后卓亲自输入其中一个同义词时，仍可创建它自己的独立词条。句子、名言和谚语不保存同义词。
- 同义词会排除当前词、标准形式、词形变化和易混词，并拒绝中文、HTML/JavaScript 或重复项；搜索同时命中独立词条和他词的同义词时，独立精确词条排在最前。
