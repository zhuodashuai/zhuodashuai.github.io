...
| QA-014 | SW 旧资源 | 明确更新、草稿 flush 后才激活、旧 cache 清除 | cache-first 可能长期保留旧 JS/CSS/manifest | Medium | mutable asset 缓存策略过强 | network-first revalidation + explicit update + old-cache cleanup | PWA runtime + E2E | PASS | PASS（Worker 与 Pages 的 v23 资源均已实测） |
