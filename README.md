...
Book-based reading lists live under `vocab/data/reading-lists/`. `Never Let Me Go` Chapters 1, 2 and 3 are kept as reviewed 29-item, 38-item and 16-item source files and imported with `node tooling/scripts/import-reading-list.mjs --source=<chapter-file>`. Chapter 3's underlined terms were manually checked against the supplied book-page photographs; the photographs themselves are not published. Examples are written for learning and are not quotations from the novel. The importer preserves the canonical term, page and form seen in the text, assigns the book/chapter membership and source, refuses ambiguous cross-book duplicates, and is safe to rerun without creating duplicates. A repeated headword with the same sense across chapters keeps one canonical card and review identity while `readingContexts` preserves each chapter's distinct form, page and explanation. In the public reader, each book and chapter has its own view and review queue; Chinese glosses and the active chapter context are displayed as separate learning points.
