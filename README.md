...
Book-based reading lists live under `vocab/data/reading-lists/`. `Never Let Me Go` Chapters 1–4 are kept as reviewed 29-item, 38-item, 28-item and 51-item source files and imported with `node tooling/scripts/import-reading-list.mjs --source=<chapter-file>`. Chapters 3 and 4 were manually checked against the supplied book-page photographs; the photographs themselves are not published. Chapter 3 contains the original 16 marked items plus 12 approved additions balancing useful expressions with writing and descriptive vocabulary. Chapter 4 covers pp. 37–48 and contains the original 11 marked items plus 40 explicitly approved additions. Supplied British IPA is preserved; unverified IPA is not invented. Notes distinguish marked text from complete learning phrases expanded from its context. Examples are written for learning and are not quotations from the novel.
