...
Book-based reading lists live under `vocab/data/reading-lists/`. `Never Let Me Go` Chapter 1 is kept as a reviewed 29-item source file and imported with `node tooling/scripts/import-reading-list.mjs`. The importer preserves the canonical term and the form seen in the text, assigns the book/chapter membership and source, refuses ambiguous cross-book duplicates, and is safe to rerun without creating duplicates. In the public reader, each book and chapter has its own view and review queue; Chinese glosses and the chapter-only context are displayed as separate learning points.
