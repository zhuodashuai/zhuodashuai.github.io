...
正式 OAuth 已真实验证为 GitHub `@zhuodashuai`、user ID `156042078` 和固定目标仓库。当前 GitHub 公开 canonical snapshot 有 4 条：`jab at`、`hip`、`surveillance` 与 Owner 本人刚发布的 `perspicacious`；本轮合并保留了 `perspicacious` 新增与随后发生的 `surveillance` 远端修订，没有覆盖或回滚。`hip` 是旧 v37 页面留下的待发布任务在 OAuth 恢复窗口中被自动提交的；它的中文、IPA 与义项内容正确，因此本轮保留，没有擅自删除。v38 已加入启动恢复屏障、页面运行实例绑定、关闭页面中止、IndexedDB v6 和 Worker 端双协议门禁，旧页面及旧队列不能再静默发布。前端静态资源缓存已升级为 v45；发布协议仍保持 v38。`bank`、`rigorous` 与生产语义实测产生的 `serendipity`、`alleviate` 仍只在当前浏览器草稿中。
...
| PWA | desktop/mobile、standalone manifest、Service Worker v45；公开页自动应用新版本，Owner 仍经两次草稿落盘后手动更新；API network-only、旧 cache 清理 |
